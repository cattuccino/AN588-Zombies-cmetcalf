...
library(dplyr)
d %>% filter(gender == "Female" | gender == "Male" | gender == "NonBinary") %>% 
...
They do seem to be related in that the majority is around 20 years of age for both variables and concentrate around a positive sloped line of best fit. Therefore, both height and weight increase with age but the weight plot does appear to have more spread.
...
hist(d$zombies_killed, main = "Histogram of Zombies Killed",
     xlab = "Zombies Killed")
```
This qqplot and histogram appear not normal, with the histogram being skewed left (lower tail) and the qqplot not falling evenly on the line at all. It likely belongs to an exponential distribution based on the ggplot rapidly increasing along a curved line rather than the straight line indicated.
...
```
Similar to the zombies killed plot, this also is not a normal distribution. The histogram is skewed towards the lower tail and the qqplot is in platforms instead of falling along the line. Again this is likely an exponential distribution, with similar reasoning stated uner zombies killed.
...
```
*struggled to find way to calculate a CI for a non-normal distribution, struggled with using the CLT, any tips? May be a Poisson distribution so as my peer commentor Sam recommended, we could use the poisson.test() function:
```{r}
poisson.test(length(subsetsample2$zombies_killed), conf.level = 0.95)
```
...
Assuming it may be a poisson distribution like zombies killed:
```{r}
poisson.test(length(subsetsample2$years_of_education), conf.level = 0.95)
```
Issue with this is it gives the same as above, so this may not be the solution we are looking for. But I will leave it as one possible way to calculate a CI for a non-normal distribution.
